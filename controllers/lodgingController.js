...
class LodgingController {
  ...
  static async showLodgingById(req, res, next) {
    const { id } = req.params;
    try {
      const lodging = await Lodging.findByPk(id, { include: [User, Type] });
      if (!lodging) {
  ...
  static async deleteBookmark(req, res, next) {
    ...
    try {
      ...
      res.status(200).json({
        message: `success delete ${deletedBookmark.Lodging.name}`,
  ...
  static async generateQr(req, res, next) {
    try {
      ...
      const response = await axios.post(
        `https://api.qr-code-generator.com/v1/create?access-token=${process.env.QR_ACCESS_TOKEN}`,
        {
          frame_name: "no-frame",
